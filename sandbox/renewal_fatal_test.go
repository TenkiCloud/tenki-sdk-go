...
// Transient / not-yet-ready mint failures must keep the loop alive so in-flight
// provisioning (cold-start edge route, a paused or still-booting session) keeps
// renewing. The loop should exit only once its context is cancelled.
