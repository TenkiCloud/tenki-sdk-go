...
type Session struct {
	...
	// The per-session edge (Caddy) route is published by the engine and applied
	...
	SourceRegistryArtifactID  string
...
func newSession(client *Client, protoSession *sandboxv1.SandboxSession) *Session {
	...
	session.OpenCode = &OpenCode{session: session}
...
func newSessionFromCreate(client *Client, resp *sandboxv1.CreateSessionResponse) *Session {
	...
	if resp.GetRouteStatus() != sandboxv1.DataPlaneRouteStatus_DATA_PLANE_ROUTE_STATUS_FAILED {
		session.configureDataPlane(resp.GetDataPlaneEndpoint(), resp.GetCredential())
		if resp.GetRouteStatus() == sandboxv1.DataPlaneRouteStatus_DATA_PLANE_ROUTE_STATUS_VERIFIED {
...
func (s *Session) apply(protoSession *sandboxv1.SandboxSession) {
	...
	s.SourceRegistryArtifactID = protoSession.GetSourceRegistryArtifactId()
...
func (s *Session) copyFrom(other *Session) {
	...
	s.SourceRegistryArtifactID = other.SourceRegistryArtifactID
...
// client. The engine publishes the Caddy route synchronously but Caddy applies
		...
		if err != nil {
			...
			default:
				if endpoint := strings.TrimSpace(resp.Msg.GetDataPlaneEndpoint()); endpoint != "" {
					s.configureDataPlane(endpoint, resp.Msg.GetCredential())
					if resp.Msg.GetRouteStatus() == sandboxv1.DataPlaneRouteStatus_DATA_PLANE_ROUTE_STATUS_VERIFIED {
...
func (s *Session) waitReadyStream(ctx context.Context, timeout time.Duration) error {
	...
	for stream.Receive() {
		updated := newSession(s.client, stream.Msg().GetSession())
		s.copyFrom(updated)
		if s.State.IsReady() {
			return nil
		}
